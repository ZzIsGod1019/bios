...
use tardis::{basic::{dto::TardisContext, result::TardisResult}, db::sea_orm::{prelude::Expr, sea_query::{Alias, SelectStatement}, EntityName, Set}, serde_json::json, TardisFuns, TardisFunsInst};

use crate::{domain::flow_model_version, dto::{flow_model_dto::{FlowModelBindStateReq, FlowModelFilterReq}, flow_model_version_dto::{FlowModelVersionAddReq, FlowModelVersionBindState, FlowModelVersionDetailResp, FlowModelVersionFilterReq, FlowModelVersionModifyReq, FlowModelVersionSummaryResp}, flow_state_dto::{FlowStateAggResp, FlowStateDetailResp, FlowStateFilterReq}}, flow_config::FlowBasicInfoManager};
...
impl RbumItemCrudOperation<flow_model_version::ActiveModel, FlowModelVersionAddReq, FlowModelVersionModifyReq, FlowModelVersionSummaryResp, FlowModelVersionDetailResp, FlowModelVersionFilterReq> for FlowModelVersionServ {
    ...
    async fn package_ext_add(id: &str, add_req: &FlowModelVersionAddReq, _: &TardisFunsInst, _ctx: &TardisContext) -> TardisResult<flow_model_version::ActiveModel> {
        Ok(flow_model_version::ActiveModel {
            ...
            current_version_sign: Set("".to_string()),
    ...
    async fn package_ext_query(query: &mut SelectStatement, _: bool, filter: &FlowModelVersionFilterReq, _: &TardisFunsInst, _: &TardisContext) -> TardisResult<()> {
        query
            .column((flow_model_version::Entity, flow_model_version::Column::CurrentVersionSign))
...
impl FlowModelVersionServ {
    fn get_current_version_sign(states: Vec<FlowStateAggResp>) -> TardisResult<String> {
        tardis::crypto::crypto_digest::TardisCryptoDigest {}.md5(
            TardisFuns::json.obj_to_string(&states.into_iter().sorted_by_key(|state| state.id.clone()).collect_vec())?
        )
    }
    ...
    async fn bind_state(flow_version_id: &str, req: &FlowModelBindStateReq, funs: &TardisFunsInst, ctx: &TardisContext) -> TardisResult<()> {
        ...
        .await
        {
            ...
            let tag = FlowModelServ::get_item(flow_version_id, &FlowModelFilterReq::default(), funs, ctx).await?.tag;
    ...
    pub async fn bind_states(flow_version_id: &str, states: Vec<FlowModelVersionBindState>, funs: &TardisFunsInst, ctx: &TardisContext) -> TardisResult<()> {
        let mut state_ids = vec![];
        for bind_state in states {
            let state_id = if let Some(exist_state_id) = bind_state.exist_state_id {
                exist_state_id  
            } else if let Some(new_state) = bind_state.new_state {
            ...
            if let Some(add_transitions) = bind_state.add_transitions {
                FlowTransitionServ::add_transitions(flow_version_id, add_transitions, funs, ctx).await?;
            }
        }
        Ok(())
    }
